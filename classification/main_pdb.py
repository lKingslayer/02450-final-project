...
def model_runner_kfold(dataDir, seeds, scoreFn, queryMethod=None, batch_size=None, shuffle=False, start_prop=0.05, end_prop=0.8):
  '''
  ...
  Args:
    ...
    shuffle (str): choices = [True, False]. Default=False (no shuffling done on embeddings)
  ...
  results = Parallel(n_jobs=3)(delayed(single_seed)(s, X, y, scoreFn)
...
for q in queryMethod:
  for b in batch_size:
    ...
    scores = model_runner_kfold(
        dataDir, seeds, f1_score, queryMethod=q, batch_size=b)
    ...
    res = {"Query": q, "Batch_Size": b, "mean_CV": CV_scores_mean, "std_CV": CV_scores_std,
           "mean_test": Classif_scores_mean, "std_test": Classif_scores_std, "Shuffle": False, "Embedding": "Both", "Score": "F1_Score"}
    results.append(res)

with open("Default_Run_F1.pkl", "wb") as f:
