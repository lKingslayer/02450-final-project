...
def strat_cross_validator(lossFn, X, y, seed, test_size=0.2, numFold=5):
  np.random.seed(seed)
...
def model_runner_logo(dataDir, seeds, scoreFn, queryMethod=None, batch_size=None, start_prop=0.01, end_prop=0.4):
  ...
  X = np.vstack(data["Embedding"].to_numpy())
  y = data["Activity"].to_numpy()
  print(
      f"input size: {X.shape} | label size: {y.shape}")
  groups = data["Clusters"].to_numpy()
  ...
  def single_seed(s, X, y, groups, scoreFn):
    ...
    if queryMethod == None:  # Run offline model on end_prop of dataset using hold-out strategy
      ...
      print(
          f"Seed: {s} | Batch size: {batch_size} | Query method: {queryMethod}")

      scores = {}
      scores["CV"] = 0
      scores["Classif_Train"] = 0
      scores["Classif_Test"] = 0

      # Define current hold-out set
      grp_idx = np.where(groups == 1)[0]
      ...
      CVscore = strat_cross_validator(
          scoreFn, X_data, y_data, s)
      scores["CV"] = CVscore
      print(
          f"Offline CV score: {CVscore} | Seed {s}")
      ...
      scores["Classif_Train"] = ClassifScore

      # Evaluate offline model using hold-out group
      y_pred = clf.predict(hold_out_set_X)
      ClassifScore = scoreFn(y_pred, hold_out_set_y)
      print(
          f"Offline Classification score for hold-out: {ClassifScore} | Seed {s}")
      scores["Classif_Test"] = ClassifScore
    ...
    '''
    print(f"Seed: {s} | Query method: {queryMethod}")

    scores = {}
    scores["CV"] = []
    scores["Classif_Train"] = []
    scores["Classif_Test"] = []
    # Define current hold-out set
    grp_idx = np.where(groups == 1)[0]
    hold_out_set_X = X[grp_idx, :]
    ...
    stop_size = int(end_prop * data_length)
    print(
        f"Seed: {s} | Query method: {queryMethod} | Batch size: {batch_size} | Pretrain size: {pretrain_size} | Stop size: {stop_size}")
    ...
    initialScore = strat_cross_validator(
        scoreFn, X_pretrain, y_pretrain, s)
    ...
    scores["Classif_Train"].append(trainscore)

    # Test the model on hold-out group

    y_pred = clf.predict(hold_out_set_X)
    testscore = scoreFn(hold_out_set_y, y_pred)
    scores["Classif_Test"].append(testscore)

    print(
        f"Seed: {s} | Query method: {queryMethod} | Batch size: {batch_size} | Pretrained model: CV: {initialScore}, Classification on train: {trainscore}, Classification on train: {testscore}")
    ...
    while curr_state < stop_size:
      ...
      if batch % 5 == 0:
        print(
            f"Seed: {s} | Query method: {q} | Progress: {curr_state}/{stop_size} | Current training score: {trainscore} | Current testing score: {testscore}")
  ...
  allScores["Mean_CV"] = []
  allScores["Std_CV"] = []
  allScores["Mean_Classif_Train"] = []
  allScores["Std_Classif_Train"] = []
  allScores["Mean_Classif_Test"] = []
  allScores["Std_Classif_Test"] = []

  grpCVSeeds = np.array([results[i]["CV"] for i in range(len(seeds))])
  grpClassifTrainSeeds = np.array(
      [results[i]["Classif_Train"] for i in range(len(seeds))])
  grpClassifTestSeeds = np.array(
      [results[i]["Classif_Test"] for i in range(len(seeds))])
  ...
  allScores["Mean_CV"] = grpCVSeeds
  allScores["Std_CV"] = grpCVStd

  allScores["Mean_Classif_Train"] = grpCVSeeds
  allScores["Std_Classif_Train"] = grpClassifTrainStd

  allScores["Mean_Classif_Test"] = grpClassifTestMean
  allScores["Std_Classif_Test"] = grpClassifTestStd
...
for q in queryMethod:
  for b in batch_size:
    ...
    res = {"Query": q, "Batch_Size": b, "mean_CV": scores["Mean_CV"], "std_CV": scores["Std_CV"],
           ...
           "mean_classif_test": scores["Mean_Classif_Test"], "std_classif_test": scores["Std_Classif_Test"], "Score": scoreName}
    results.append(res)

with open(f"Default_Run_init_{scoreName}.pkl", "wb") as f:
